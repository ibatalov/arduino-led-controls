...
void WS2812B::init(uint16_t num_rgb, uint16_t num_bytes, uint8_t digital_pin) {
  ...
  if(rgb_arr){
    free(rgb_arr); // this should never happen unless I mess up. One init() call per whole program run!
  }
  
  rgb_arr = (uint8_t *)malloc(num_bytes);
  Serial.print(F("LED strip init(). Num bytes taken: "));
